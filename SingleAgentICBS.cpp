...
bool SingleAgentICBS::findPathPrioritized(bool goal_optimal, vector<PathEntry> &path, int &goal_length, double f_weight, const vector < list< tuple<int, int, bool> > >* constraints, const bool* res_table, size_t max_plan_len, double lowerbound) {
    // clear data structures if they had been used before
    // (note -- nodes are deleted before findPath returns)
